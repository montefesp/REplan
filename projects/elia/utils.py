...
def upgrade_topology(net: pypsa.Network, regions: List[str], plot: bool = False) -> pypsa.Network:
    ...
    buses = pd.DataFrame(columns=["region", "x", "y", "country", "onshore"])
    links = pd.DataFrame(columns=["bus0", "bus1", "carrier"])

    if "IS" in regions:
        buses.loc["IS", "region"] = get_shapes(["IS"], "onshore")["geometry"][0]
        buses.loc["IS", ["x", "y"]] = buses.loc["IS", "region"].centroid
        buses.loc["IS", "country"] = "IS"
        buses.loc["IS", "onshore"] = True
        # Adding link to GB
    ...
    if "GL" in regions:
        ...
        # TODO: probably need to update that not to take the full shape -> stop at some latitude
    ...
    if "NA" in regions:
        ...
        for c in countries:
            buses.loc[c, "region"] = shapes.loc[c]
            buses.loc[c, "country"] = c
            buses.loc[c, "onshore"] = True
